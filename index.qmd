...
cantril_clean <- 
  ...
  mutate(`Absolute change` = round((`2023`-`2011`), 2),
         `Relative change` = percent(round((`2023`-`2011`)/`2011`, 2)))
  
reactable(cantril_clean)
...
# Filter 8 African countries
africa_scores <- cantril %>% 
  filter(Country %in% c("Tanzania", "Kenya", "Egypt", "Libya", 
                        "South Africa", "Ghana", "Rwanda", "Uganda"))
#africa_scores
...
line_plot <- 
  ...
  labs(
    title = "Perception of happiness among African countries.",
    subtitle = "Cantril ladder scores.",
    y = "Cantril score") +
  theme_minimal()
...
## Happiest vs. Least Happy Countries
...
# Identify countries with least scores:

#Least happy: 
#Afghanistan, Lebanon, Lesotho, Sierra Leone, Democratic Republic of Congo, Zimbabwe, Botswana, Malawi, Eswatini, Zambia
...
Hap_sad_countries <- 
  cantril %>% 
  filter(Country %in% 
           c("Afghanistan", "Lebanon", "Lesotho", "Sierra Leone", 
             "Democratic Republic of Congo", "Zimbabwe", "Botswana", 
             "Malawi", "Eswatini", "Zambia", 
             "Finland", "Denmark", "Iceland", "Sweden", "Israel", 
             "Netherlands", "Norway", "Luxembourg", "Switzerland", "Australia")) %>% 
  filter(Year == 2023) %>% 
  ggplot(aes(x = reorder(Country, `Cantril ladder score`), 
             y = `Cantril ladder score`, 
             fill = Country)) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  scale_y_continuous(breaks = c(0,2,4,6,8,10))+
  labs(title = "Top 10 and Bottom 10 Countries by Happiness Score (2023)",
       caption = "World Happiness Report (2023)",
       x = "Country",
       y = "Happiness Score")+
  theme_minimal()
